...
def lambda_handler(event, context):
    ...
    try:
        ...
        validation_result = perform_security_validations(payload, user_ip, event)
...
def perform_security_validations(payload, user_ip=None, event=None):
    """
    ...
    return validate_cognito_groups_access_with_policies(cognito_groups, current_hour, user_ip, event)
...
def validate_api_access(allowed_apis, current_endpoint):
    """
    Valida si el endpoint actual está permitido para el grupo
    """
    if '*' in allowed_apis:
    ...
    for allowed_api in allowed_apis:
        if allowed_api.endswith('*'):
            # Patrón con wildcard (ej: /products/*)
            prefix = allowed_api[:-1]
            if current_endpoint.startswith(prefix):
                return True
        elif allowed_api == current_endpoint:
            # Coincidencia exacta
            return True
    
    return False

def validate_cognito_groups_access_with_policies(cognito_groups, current_hour, user_ip=None, event=None):
    """
    ...
    for group in cognito_groups:
        if group in GROUP_POLICIES:
            ...
            if event:
                current_endpoint = f"/{event.get('resource', '').lstrip('/')}"
                if not validate_api_access(group_policy['apis'], current_endpoint):
                    return {
                        'allowed': False, 
                        'reason': f'Access denied: {current_endpoint} not allowed for group {group}. Allowed APIs: {group_policy["apis"]}'
                    }
                logger.info(f"API access validated for {current_endpoint} by group {group}")
